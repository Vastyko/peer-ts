  ...
  paths: {
    "/api/pauth/authorize?response_type={response_type}&scope={scope}&client_id={client_id}&redirect_uri={redirect_uri}&state={state}&platform={platform}": {
      get: {
        ...
        tags: ["2. 账号接口"],
        operationId:
          "ApiPauthAuthorizeClientIdClientIdRedirectUriRedirectUriStateStatePlatformPlatformByScopeGet",
        produces: ["application/json"],
        parameters: [
          {
            name: "response_type",
          ...
          },
          {
            ...
            type: "string",
            description:
              "表示重定向回调地址的uri。uri必须符合创建客户端时的callbackUri设置规则。"
          },
          {
            name: "state",
            in: "path",
          ...
          },
          {
            ...
            type: "string",
            description:
              "平台，用于给前端进行UI适配。app：移动客户端，pc-app：桌面客户端，maxhub：MAXHUB，web：桌面端浏览器，weixin：移动端微信，mobile：移动端浏览器。默认显示移动端微信版面。"
          }
        ],
        responses: {
          "200": {
          ...
          "200": {
            ...
            examples: {
              "application/json": [
                {
                  downloadUrl:
                    "res.test.maxhub.vip/share.html?s_id=0f205e93-a737-4267-a33d-b44c4d8941c0&locale=zh_CN#/detail/5a66f2ba-85d3-48ee-b6ad-f342ce3b1eaf/record",
                  _id: "cb5222accd2acec",
                  meeting: "所处的会议 id",
                  name: "文件名",
                  createTime: "1532930897000",
      ...
      get: {
        ...
        parameters: [
          ...
          },
          {
            ...
            exclusiveMinimum: false,
            description:
              "\b消息的页数，page 和 row 必须同时设置（当前版本暂不实现）"
          },
          {
            name: "row",
            in: "path",
            ...
            exclusiveMinimum: false,
            description:
              "每页消息的个数，page 和 row 必须同时设置（当前版本暂不实现）"
          }
        ],
        responses: {
          "200": {
  ...
  definitions: {
    ...
    RepeatMeeting: {
      ...
      properties: {
        ...
        repeatValue: {
          description:
            "如果重复类型为日，则表示间隔的天数，至少间隔1天。如果重复类型为周，则表示重复的星期，值为1~7",
          example: 1,
          type: "number",
          format: "double"
        },
